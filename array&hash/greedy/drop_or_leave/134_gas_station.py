# same as: the first element which negative->positive

# Optimize. TODO: try compare to 678: valid parenthesis string
class Solution:
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        ...
        for i in range(len(gas)):
            total += (gas[i]-cost[i])
            if total<0: # remove the possible ans (starting point) at index before i 
                total=0
                res=i+1
        return res
# Explain
